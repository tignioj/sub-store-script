async function operator(proxies = [], targetPlatform, context) {
  ...
  // 从订阅源 URL 中自动提取 service 和 id 参数
  // 例如订阅链接为 https://jmssub.net/members/getsub.php?service=123&id=xxx 时自动识别
  let serviceId = 'YOUR_SERVICE_ID' // 默认值（当订阅 URL 不包含参数时使用）
  let userId = 'YOUR_USER_ID' // 默认值（当订阅 URL 不包含参数时使用）
  ...
  try {
    for (const [key, value] of Object.entries(context.source)) {
      if (!key.startsWith('_') && value.url) {
        ...
        if (urlService) serviceId = urlService
        if (urlId) userId = urlId
        break
      }
    }
  } catch {
